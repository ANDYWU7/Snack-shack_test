const snacks = [
  {id:'cookie',name:'Chocolate chip cookie',description:'Golden edges. Gooey middle.',price:150,emoji:'🍪',color:'#eee2cf',tag:'THE CROWD FAVORITE'},
  {id:'chips',name:'Classic potato chips',description:'A little salty. A lot of crunch.',price:100,emoji:'🥔',color:'#f9e6ab'},
  {id:'brownie',name:'Fudge brownie',description:'For your chocolate fix.',price:200,emoji:'🍫',color:'#e9cdbc',tag:'SWEET TOOTH APPROVED'},
  {id:'pretzels',name:'Mini pretzels',description:'Small twists. Big crunch.',price:100,emoji:'🥨',color:'#eddbbb'},
  {id:'gummies',name:'Fruit gummies',description:'A pocketful of fruity goodness.',price:100,emoji:'🍓',color:'#e8ecd0'},
  {id:'juice',name:'Juice box',description:'A refreshing little sidekick.',price:100,emoji:'🧃',color:'#d9e8e1'}
];
...
function renderMenu() {
  grid.innerHTML = snacks.map(snack => `<article class="snack-card"><div class="snack-image" style="--card-bg:${snack.color}">${snack.image ? `<img src="${snack.image}" alt="${snack.name}" width="600" height="400">` : `<span class="emoji" aria-hidden="true">${snack.emoji}</span>`}${snack.tag ? `<span class="snack-tag">${snack.tag}</span>` : ''}</div><div class="card-content"><h3>${snack.name}</h3><p class="card-description">${snack.description}</p><div class="card-bottom"><span class="price">${money(snack.price)}</span><button class="add-button" data-add="${snack.id}" aria-label="Add ${snack.name}">Add <span aria-hidden="true">+</span></button></div></div></article>`).join('');
}
function summary() {
...
if(document.modelContext?.registerTool){try{Promise.resolve(document.modelContext.registerTool({name:'set_snack_quantity',title:'Set snack quantity',description:'Update this page’s local snack list. Does not submit or reserve an order.',inputSchema:{type:'object',properties:{id:{type:'string',enum:snacks.map(s=>s.id)},quantity:{type:'integer',minimum:0,maximum:99}},required:['id','quantity'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},execute(input){if(!input||!Number.isInteger(input.quantity)||input.quantity<0||input.quantity>99||!snacks.some(s=>s.id===input.id))throw new Error('Invalid snack or quantity');return changeQuantity(input.id,input.quantity-(cart.get(input.id)||0))}})).catch(()=>{});}catch{}}
